(function () {
    ...
    function enhanceCard(card, picking, context) {
        ...
        const validationDisabled = Number(picking.receivedQuantity) <= 0;
        ...
        context.replacePrimaryActions([
            ...
            context.createValidationAction("main-action validation-action", validationDisabled),
        ]);
    }

    async function beforeValidate(picking) {
        if (Number(picking.receivedQuantity) > 0) {
            return true;
        }

        inventory.showToast("É necessário informar uma quantidade recebida maior que zero antes de validar.", "!");
        return false;
    }
